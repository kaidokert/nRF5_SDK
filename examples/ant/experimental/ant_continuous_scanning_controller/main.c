...
#define ANT_BS_CHANNEL_TYPE         CHANNEL_TYPE_SLAVE       /**< Channel type background scanning (Bi-directional slave). */
#define ANT_BS_DEVICE_NUMBER        ((uint16_t) 0)           /**< Device number (wild-card). */

#define ANT_DEVICE_TYPE             ((uint8_t) 1)            /**< Device type. */
#define ANT_TRANSMISSION_TYPE       ((uint8_t) 0x15)         /**< Transmission type. */
#define ANT_FREQUENCY               ((uint8_t) 25)           /**< Radio frequency. */

#define MAX_DEVICES                 ((uint8_t) 16)           /**< Maximum number of nodes supported in this network. */
#define ANT_HOME_ID                 ((uint16_t) 0x1234)      /**< Unique ID for the entire network. */
 ...
 */
static void start_command_sequence(uint8_t command)
{
    uint32_t err_code;
...
static void bsp_evt_handler(bsp_event_t evt)
{
    switch (evt)
    {
        case BSP_EVENT_KEY_0:
            LEDS_OFF(BSP_LED_2_MASK);
            start_command_sequence(COMMAND_STATE_ON);
            break;

        case BSP_EVENT_KEY_1:
            LEDS_OFF(BSP_LED_2_MASK);
            start_command_sequence(COMMAND_STATE_OFF);
            break;

        case BSP_EVENT_KEY_2:
            LEDS_OFF(BSP_LED_2_MASK);
            start_command_sequence(COMMAND_STATE_ALL_ON);
            break;

        case BSP_EVENT_KEY_3:
            LEDS_OFF(BSP_LED_2_MASK);
            start_command_sequence(COMMAND_STATE_ALL_OFF);
            break;
