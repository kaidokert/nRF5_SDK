...
#define CHAN_ID_DEV_TYPE            0x02                    /**< Device type. */
#define CHAN_ID_TRANS_TYPE          0x01                    /**< Transmission type. */
#define DEVICE_NUMBER               (NRF_FICR->DEVICEID[0]) /**< Device number. */

// Channel period configuration.
#define CHAN_PERIOD                 8192                    /**< Channel period. */

// Search time-out configuration (search time-out in seconds = CHAN_SEARCH_TIMEOUT * 2.5).
#define CHAN_SEARCH_TIMEOUT         255                     /**< Search time-out. */

// RF frequency configuration.
#define RF_FREQ                     76                      /**< RF frequency. */
...
// Encryption defines.
/** Encryption ID. */
#define CRYPTO_ID                   {(0xFF00 & DEVICE_NUMBER) >> 8, (0xFF & DEVICE_NUMBER), \
                                     (0xFF00 & DEVICE_NUMBER) >> 8, (0xFF & DEVICE_NUMBER)}
...
void ant_se_channel_rx_broadcast_setup(void)
{
    ...
    for (int i = 0; i < NUMBER_OF_CHANNELS_TO_OPEN; i++)
    {
