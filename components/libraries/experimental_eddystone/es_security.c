...
static void eid_generate(uint8_t slot_no)
{
    ...
    uint32_t k_bits_cleared_time =
        (m_security_slot[slot_no].timing.time_counter >>
    m_security_slot[slot_no].timing.k_scaler) << m_security_slot[slot_no].timing.k_scaler;
...
static void es_beacon_ecdh_pair_generate(uint8_t * p_priv_buffer, uint8_t * p_pub_buffer)
{
    ...
    if (pool_size < ESCS_ECDH_KEY_SIZE)
    {
        ...
        (void)sd_rand_application_vector_get(p_priv_buffer + pool_size, ESCS_ECDH_KEY_SIZE - pool_size);
    }
...
void es_security_update_time(void)
{
    ...
    uint32_t second_since_last_invocation = es_security_timing_seconds_passed();
                                               ...
                                               uint8_t   scaler_k)
{
    ...
    return es_security_ecdh_pair_preserve();
}
...
{
    return es_flash_access_ecdh_key_pair(m_ecdh.ecdh_key_pair.private,
...
{
    p_config->frame_type = ES_FRAME_TYPE_EID;
    p_config->k_scaler   = m_security_slot[slot_no].timing.k_scaler;
    p_config->time_counter    = m_security_slot[slot_no].timing.time_counter;
    memcpy(p_config->ik, m_security_slot[slot_no].aes_ecb_ik.key, ESCS_AES_KEY_SIZE);
}
...
ret_code_t es_security_init(es_security_msg_cb_t security_callback)
{
    ...
    uint8_t pub_key_buff[ESCS_ECDH_KEY_SIZE];
    uint8_t priv_key_buff[ESCS_ECDH_KEY_SIZE];

    err_code = es_flash_access_ecdh_key_pair(priv_key_buff,
                                             pub_key_buff,
    ...
    if (err_code != FDS_ERR_NOT_FOUND)
    {
        memcpy(m_ecdh.ecdh_key_pair.private, priv_key_buff, ESCS_ECDH_KEY_SIZE);
        memcpy(m_ecdh.ecdh_key_pair.public, pub_key_buff, ESCS_ECDH_KEY_SIZE);
        m_security_callback(0, ES_SECURITY_MSG_ECDH);
    }
